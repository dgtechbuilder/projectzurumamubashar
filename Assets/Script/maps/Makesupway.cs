...
// Main class creates entire map system - items and obstacles, random obstacles, map
public class Makesupway : MonoBehaviour
{


    [Header("Forest Patches ")]
    public GameObject forestpatch1;
    public GameObject forestpatch2;
    [Header("Desert Patches ")]
    public GameObject desertpatch1;
    public GameObject desertpatch2;
    [Header("Village Patches ")]
    public GameObject villagepatch1;
    public GameObject villagepatch2;
    [Header("=== MAIN MAP PATCHES ===")]
    [HideInInspector] public GameObject main, m1, m2, m3, m4, m5; // Not used - only stored in mapmain list which is never accessed

    [Header("=== MAP PREFABS (For Instantiation) ===")]
    public GameObject makesupway; // Railway track
    [HideInInspector] public GameObject makesupwaymap1; //  map 1 (Not used - case 0 uses Map1 instead)
    [HideInInspector] public GameObject makesupwaymap2; //  map 2 (Not used - case 1 uses Map2 instead)
    [HideInInspector] public GameObject makesupwaymap3; //  map 3 (Disabled - cases 2,3,4 disabled)
    enum stylecreate { left, betten, righ }
    stylecreate newstyle;

    [Header("=== OBSTACLE PREFABS ===")]
    private GameObject makeemtyshipdie; // Intermediate train car
    public GameObject makeemtyshipdie1; // Train car 2
    public GameObject makeemtyshipdie2; // Train car 2
    public GameObject makeemtyshipdiemuving; // Train car 1 rushing towards player
    public GameObject makeemtyship; // Create train car with sand
    public GameObject makeembars_smore; // Low barrier
    public GameObject makeembars_big; // High barrier
    public GameObject makeembars_biger; // Large barrier
    public GameObject makeembridge; // Bridge middle leg
    public GameObject makeembridgeleft; // Bridge right leg
    public GameObject makeembridgerigh; // Bridge left leg
    public GameObject makeembridgebehig; // Bridge legs on both sides
    public GameObject powerpoles; // Power poles
    Vector3 locationemty = new Vector3(0, 0, 0); // Store obstacle creation position

    [Header("=== ITEM PREFABS ===")]
    public GameObject makeitemcoin; // Main coin
    public GameObject makeitemhutcoin; // Coin magnet
    public GameObject makeitembay; // Short flight
    public GameObject makeitembaylong; // Long flight
    public GameObject makeitemnhay; // High jump shoes
    public GameObject makeitemx2coin; // X2 coin
    public GameObject makeitemvan; // Skateboard
    public GameObject makeitembox; // Gift box
    public GameObject makeitemkey; // Key
    public GameObject makeitemsword; // Sword (spawns once per game)
    
    // Track if sword has spawned (only once per game)
    public static bool swordHasSpawned = false;
    GameObject bettwen;
    int randummap;
    int randumemty;
    ...
    public GameObject Map1; // Case 0 - Active
    public GameObject Map2; // Case 1 - Active
    [HideInInspector] public GameObject Map3; // Not used
    public GameObject Map4; // Still referenced in active code (waterfall checks)
    [HideInInspector] public GameObject Map5; // Disabled - Case 6 disabled
    [HideInInspector] public GameObject Map6hamcho; // Disabled - Case 5 disabled
    [HideInInspector] public GameObject Map42; // Not used
    // Create random map
    int valueofmap;
    public static bool isnewgame = false; // Check first play session
    ...
    void Start()
    {
        ...
        makemap = this;
        valueofmap = 0; // Cycle: 0=Desert1, 1=Desert2, 2=Village1, 3=Village2
        
        // Initialize first maps in the cycle (Desert Patch 1)
        Map1 = desertpatch1;
        Map2 = desertpatch2;
        
        // Reset sword spawn flag for new game
        swordHasSpawned = false;
    ...
    /// <summary>
    /// Select map theme and assign to Map1 and Map2
    /// Saves selection to PlayerPrefs (scene will restart after selection)
    /// </summary>
    /// <param name="value">0=Forest, 1=Desert, 2=Village</param>
    public void SelectMap(int value)
    {
        // Validate value range
        if (value < 0 || value > 2)
        {
            value = 0; // Default to Forest if invalid
        }
        
        // Assign selected maps based on value
        if (value == 0)
        {
            Map1 = forestpatch1;
            Map2 = forestpatch2;
        ...
        // Save selected map index to PlayerPrefs
        PlayerPrefs.SetInt("SelectedMapIndex", value);
        PlayerPrefs.Save();

        // Note: Scene will restart after this, so no need for runtime reinitialization

       // SceneManager.LoadScene("mainlv original");
    }
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            switch (value)
            {
                ...
                case 1:
                    ...
                    break;
                // TEMPORARILY DISABLED - Cases 2, 3, 4 (Prefab Instantiation)
                // case 2:
            ...
            // TEMPORARILY DISABLED - Skip cases 2, 3, 4
            if (i >= 2)
            {
                // Skip to next valid case (5) since 2, 3, 4 are disabled
                continue; // Skip cases 2, 3, 4
            }
            for (int j = 1; j < m; j++)
            {
                ...
                if (makesupway.gameObject != null)
                {
                    switch (value)
                    {
                        case 0:
                            if (i == 0)
                            {
                                // Initialize with Desert Patch 1
                                Map1 = desertpatch1;
                                if (Map1 != null && Map1.transform.position.z < 0)
                                {
                                    Map1.SetActive(true);
                        ...
                        case 1:
                            if (i == 1)
                            {
                                // Initialize with Desert Patch 2
                                Map2 = desertpatch2;
                                if (Map2 != null && Map2.transform.position.z < 0)
                                {
                                    Map2.SetActive(true);
    ...
    int notshow;
    /// <summary>
    /// Main map for player when initializing at start
    /// </summary>
    ...
    /// <summary>
    /// Reuse map segments and obstacles
    /// </summary>
    /// <param name="value"></param>
    /// <returns></returns>
    IEnumerator randumallmap(int value)
    {
        // Cycle through: 0=Desert1, 1=Desert2, 2=Village1, 3=Village2
        switch (valueofmap)
        {
            case 0: // Desert Patch 1
                Map1 = desertpatch1;
                Map1.SetActive(true);
                Map1.transform.position = new Vector3(0, 0, location.z);
                location.z += 80;
                break;
            case 1: // Desert Patch 2
                Map2 = desertpatch2;
                Map2.SetActive(true);
                Map2.transform.position = new Vector3(0, 0, location.z);
                location.z += 80;
                break;
            case 2: // Village Patch 1
                Map1 = villagepatch1;
                Map1.SetActive(true);
                Map1.transform.position = new Vector3(0, 0, location.z);
                location.z += 80;
                break;
            case 3: // Village Patch 2
                Map2 = villagepatch2;
                Map2.SetActive(true);
                Map2.transform.position = new Vector3(0, 0, location.z);
                location.z += 80;
                break;
            // TEMPORARILY DISABLED - Cases 2, 3, 4 (Prefab Instantiation Lists)
            // case 2:
            ...
            //     }
            //     break;
            // TEMPORARILY DISABLED - Cases 5, 6, 7 (Special Maps)
            // case 5:
        ...
        // Cycle through: 0=Desert1, 1=Desert2, 2=Village1, 3=Village2, then loop back to 0
        if (valueofmap >= 4)
        {
            valueofmap = 0; // Loop back to Desert Patch 1
        }
        ...
        yield return null; // Required for IEnumerator - allows coroutine to work properly
    }
    ...
    [Header("=== WATERFALL OBSTACLES ===")]
    public GameObject tha1, th2, th3;

    /// <summary>
    /// Random obstacles in waterfall
    /// </summary>
    public void Getthac()
    {
    ...
    /// <summary>
    /// Function to randomize obstacles
    /// </summary>
    public void randumtheemty()
    {
        if ((checkshow < Map4.transform.position.z + 10 && checkshow > Map4.transform.position.z - 120)) // Don't show obstacles in waterfall
        {
            Debug.Log("");
            Getthac();
            return;
    ...
    int lasrandum = 0;
    #region System to create random obstacle segments
    float DistanceTranslate = 0; // Distance needed to travel
    ...
    float delayy = 0;
    /// <summary>
    /// Create tutorial map for first time playing
    /// </summary>
    /// <param name="tranformz"></param>
    /// <returns></returns>
    ...
    /// <summary>
    /// CREATE COINS SPECIFIC TO EACH MAP
    /// </summary>
    ...
    /// <summary>
    /// Create coins for short flight item
    /// </summary>
    ...
    /// <summary>
    /// Create coins for long flight item
    /// </summary>
    /// <param name="tranformzst">Z coordinate</param>
    /// <returns></returns>
    ...
    /// <summary>
    /// Create item at fixed position, randomize item type
    /// </summary>
    ...
    public IEnumerator createallitem(float transformz, createitemposition stylecreate)
    {
        ...
        // Check if sword hasn't spawned yet - give it a chance to spawn
        if (!swordHasSpawned)
        {
            // 10% chance to spawn sword (once per game)
            int swordChance = Random.Range(0, 10);
            if (swordChance == 0)
            {
                nameitem = "sword";
                swordHasSpawned = true; // Mark as spawned
            }
            else
            {
                // Normal random item selection
                int rancoin = Random.Range(0, 8);
                switch (rancoin)
                {
                    case 0:
                        nameitem = "x2coin";
        ...
        {
            // Sword already spawned, use normal random selection
            int rancoin = Random.Range(0, 8);
            switch (rancoin)
            {
                case 0:
        ...
        switch (nameitem)
        {
            ...
            case "van":
                ...
                break;
            case "sword":
                if (makeitemsword != null)
                {
                    GameObject swordInstance = Instantiate(makeitemsword, tranformitem, transform.rotation) as GameObject;
                    ManagerAllitem.Add(swordInstance);
                    Debug.Log("Sword spawned at position: " + tranformitem + " with tag: " + swordInstance.tag);
                    
                    // Verify collider setup
                    Collider swordCollider = swordInstance.GetComponent<Collider>();
                    if (swordCollider != null)
                    {
                        Debug.Log("Sword has collider - IsTrigger: " + swordCollider.isTrigger);
                    }
                    else
                    {
                        Debug.LogWarning("Sword prefab is missing a Collider component!");
                    }
                }
                else
                {
                    Debug.LogWarning("makeitemsword prefab is not assigned in Makesupway component!");
                }
                break;
            default:
                break;
    ...
    float muving = -300;
    /// <summary>
    /// Move all objects backwards when having skateboard item or key
    /// </summary>
    /// <returns></returns>
    ...
    /// <summary>
    /// Move entire map backwards when dead - not enough keys - didn't select key
    /// </summary>
    /// <returns></returns>
