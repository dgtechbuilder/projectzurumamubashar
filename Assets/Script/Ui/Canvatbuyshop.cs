...
public class Canvatbuyshop : MonoBehaviour {
    ...
    void showCostCharacter(string namecharacter)
    {
        ...
        for (int i = 0; i < itemcharacter.Count; i++)
        {
            if (itemcharacter[i].name == namecharacter)
            {
                characterIndex = i;
                break;
            }
        ...
        bool isSelected = (characterIndex == selectedIndex);
        
        // Check if character is owned based on name (for backward compatibility)
        bool isOwned = false;
        switch (namecharacter)
        {
            case "nvchinh":
                isOwned = managerdata.manager.GetCharacternvchinh() == 1;
                if (!isOwned)
                {
                    cost.text = "95000";
                    return;
                ...
                break;
            case "nvgirl":
                isOwned = managerdata.manager.GetCharacternvgirl() == 1;
                if (!isOwned)
                {
                    cost.text = "75000";
                    return;
                ...
                break;
            case "nvgau":
                isOwned = managerdata.manager.GetCharacternvGau() == 1;
                if (!isOwned)
                {
                    cost.text = "66000";
                    return;
                ...
                break;
            default:
                // For characters 4-6, assume owned if in list (or add specific checks)
                isOwned = true;
                break;
        }
        
        // Set button text based on selection status
        if (isSelected)
        {
            cost.text = "SELECTED";
        }
    ...
    public void ClicBuyvan()
    {
        ...
        else
        {
            ...
            if (strcheck == "SELECT")
            {
                ...
                for (int i = 0; i < itemcharacter.Count; i++)
                {
                    if (itemcharacter[i].name == bettent.name)
                    {
                        characterIndex = i;
                        break;
                    }
                ...
                if (characterIndex >= 0)
                {
                    managerdata.manager.SetSelectedCharacterIndex(characterIndex);
                    // Also save character name for backward compatibility
                ...
                if (int.Parse(strcheck) <= managerdata.manager.Getcoin())
                {
                    ...
                    switch (bettent.name)
                    {
                        ...
                        case "nvgau":
                            ...
                            break;
                        default:
                            // For characters 4-6, you may want to add specific save methods
                            break;
                    }
                    cost.text = "SELECTED";
                }
